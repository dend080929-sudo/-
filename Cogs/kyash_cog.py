...
class Vending(commands.Cog):
    ...
    def _load_sessions(self):
        return load_json_store("kyash_accounts", KYASH_SESSION_FILE)

    def _save_session(self, user_id, data):
        sessions = self._load_sessions()
        sessions[str(user_id)] = data
        save_json_store("kyash_accounts", sessions, KYASH_SESSION_FILE)
    ...
    async def kyash_login(self, interaction: discord.Interaction, email: str, password: str):
        ...
        try:
            ...
            await interaction.followup.send("📲 SMSに届いた6桁のコードを `/キャッシュ認証` で入力してください。", ephemeral=True)
    ...
    async def kyash_verify(self, interaction: discord.Interaction, otp: str):
        ...
        if not attempt:
            return await interaction.followup.send("⚠️ 先に `/キャッシュログイン` を実行してください。", ephemeral=True)
        ...
        try:
            ...
            await interaction.followup.send("✅ ログイン成功！セッションが保存されました。", ephemeral=True)
        except Exception as e:
            await interaction.followup.send(f"❌ 認証失敗: {e}", ephemeral=True)
