...
async function revokeVip(publicId,username){if(!confirm(`${username}（${publicId}）のVIPを剥奪しますか？`))return;try{await api('/autocat/api/admin/vip/revoke',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-Token':config.csrfToken},body:JSON.stringify({public_id:publicId})});setStatus('VIPを剥奪しました。');await load(false,true);}catch(error){setStatus(error.message,true);}}
...
async function renameAccount(item){
  ...
  try{await api('/autocat/api/admin/accounts/update',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-Token':config.csrfToken},body:JSON.stringify({public_id:item.public_id,action:'rename',username:username.trim()})});setStatus('ユーザー名を変更しました。');await load(false,true);}catch(error){setStatus(error.message,true);}
}
...
function base64UrlToUint8Array(value){const padding='='.repeat((4-value.length%4)%4);const raw=atob((value+padding).replace(/-/g,'+').replace(/_/g,'/autocat/'));return Uint8Array.from([...raw].map(ch=>ch.charCodeAt(0)));}
async function notificationState(){if(!('serviceWorker'in navigator)||!('PushManager'in window)||!('Notification'in window)||!config.vapidPublicKey){notifyButton.disabled=true;notifyButton.textContent='通知非対応';return;}const registration=await navigator.serviceWorker.register('/autocat/account-sw.js',{scope:'/autocat/'});const subscription=await registration.pushManager.getSubscription();if(subscription)await api('/autocat/api/account/push/subscribe',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-Token':config.csrfToken},body:JSON.stringify({admin:true,subscription:subscription.toJSON()})});notifyButton.classList.toggle('on',Boolean(subscription));notifyButton.textContent=subscription?'DM通知オン':'DM通知をオン';return {registration,subscription};}
notifyButton.addEventListener('click',async()=>{try{const current=await notificationState();if(!current)return;if(current.subscription){await api('/autocat/api/account/push/subscribe',{method:'DELETE',headers:{'Content-Type':'application/json','X-CSRF-Token':config.csrfToken},body:JSON.stringify({admin:true,endpoint:current.subscription.endpoint})});await current.subscription.unsubscribe();setStatus('管理者DM通知をオフにしました。');}else{const permission=await Notification.requestPermission();if(permission!=='granted')throw new Error('通知が許可されませんでした。');const subscription=await current.registration.pushManager.subscribe({userVisibleOnly:true,applicationServerKey:base64UrlToUint8Array(config.vapidPublicKey)});await api('/autocat/api/account/push/subscribe',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-Token':config.csrfToken},body:JSON.stringify({admin:true,subscription:subscription.toJSON()})});setStatus('管理者DM通知をオンにしました。');}await notificationState();}catch(error){setStatus(error.message,true);}});
