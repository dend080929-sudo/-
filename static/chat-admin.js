...
function restrictionNode(item){
  ...
  clear.addEventListener('click',async()=>{if(!confirm(`${item.target_name} の制限を解除しますか？`))return;try{await api('/autocat/api/chat/admin/clear-restriction',{token:item.token});setStatus('制限を解除しました。','success');await load();}catch(error){setStatus(error.message,'error');}});
  row.appendChild(clear);return row;
}

async function load(){
  try{
    const data=await api('/autocat/api/chat/admin/state');
...
form.addEventListener('submit',async event=>{
  ...
  try{const data=await api('/autocat/api/chat/admin/unban-code',{code:codeInput.value});setStatus(`${data.target_name} のBANを解除しました。`,'success');codeInput.value='';await load();}
