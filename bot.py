...
async def help_cmd(interaction: discord.Interaction):
    ...
    embed.add_field(name="パネル設置系", value="`/setup_verify` : 認証パネルを設置\n`/setup_ticket` : チケットパネルを設置\n`/setup_vending` : 自販機パネルを設置(名前指定可能)", inline=False)
    embed.add_field(name="自販機管理系", value="`/vending_list` : 指定した自販機の登録商品一覧確認\n`/vending_delete` : 指定した自販機の特定商品を削除", inline=False)
...
async def setup_ticket(interaction: discord.Interaction):
    if not has_admin_role(interaction.user):
        await interaction.response.send_message("❌ 管理者権限が必要です。", ephemeral=True)
        return
    await interaction.channel.send(embed=discord.Embed(title="🎫 おお問い合わせ", description="チケットを作成するには下のボタンを押してください。", color=0x57F287), view=TicketView())
...
@app_commands.describe(machine_name="この自販機の名前（識別名。例: メイン自販機、素材屋など）")
@app_commands.checks.has_permissions(administrator=True)
...
@bot.tree.command(name="vending_list", description="指定した自販機の登録商品一覧とIDを確認します。")
...
async def vending_list(interaction: discord.Interaction, machine_name: str):
    ...
    if not items:
        embed.description = "この自販機には商品が登録されていないか、名前が間違っています。"
    for i_id, info in items.items():
        stock_val = info.get('stock', 99)
        stock_str = "∞" if stock_val == 99 else str(stock_val)
...
async def vending_delete(interaction: discord.Interaction, machine_name: str, item_id: str):
    ...
    if machine_name in data and item_id in data[machine_name]:
        ...
        await interaction.response.send_message(f"❌ 指定された自販機名または商品IDが見つかりませんでした。`/vending_list` で確認してください。", ephemeral=True)
