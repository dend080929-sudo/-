...
# -------------------------------------------------------------
# 1. 認証用Webサーバー（リニューアルされたおしゃれな完了画面）
# -------------------------------------------------------------
@app.route("/")
def home():
...
class TicketView(discord.ui.View):
    ...
    async def create_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = discord.Embed(
            title="🎫 お問い合わせチケット",
            description=f"{interaction.user.mention}さん、ようこそ！\nスタッフが対応いたしますので、用件を詳しく記入してお待ちください。",
            color=0x2b2d31
        )
...
class ItemAddModal(discord.ui.Modal, title="自販機への商品追加"):
    ...
    item_desc = discord.ui.TextInput(label="商品説明（誰でも見えます）", style=discord.TextStyle.paragraph, placeholder="例: ツムツムの自動代行ソースコードです。", max_length=500)
...
async def call_users(ctx):
    ...
    await status_msg.edit(content=f"✅ コール処理が完了しました！\n🎯 成功: {success_count}人\n❌ 失敗: {fail_count}人")
...
async def help_cmd(ctx):
    ...
    embed.add_field(name="1. `!recreate`", value="現在のチャンネルを削除し、同じ設定で新しく作り直します。", inline=False)
    embed.add_field(name="2. `!panel_embed [タイトル] [説明文]`", value="埋め込みパネルを送信します。", inline=False)
    embed.add_field(name="3. `!update_achievement`", value="実績チャンネルのカウントを手動で同期します。", inline=False)
    embed.add_field(name="4. パネル設置用コマンド", value="`!setup_verify` (認証)\n`!setup_ticket` (チケット)\n`!setup_vending` (自販機)", inline=False)
    embed.add_field(name="5. `!vending_delete [商品ID]`", value="指定したIDの自販機商品を削除します。", inline=False)
    embed.add_field(name="6. `!vending_list`", value="登録商品一覧とIDを確認します。", inline=False)
    embed.add_field(name="7. `!call_users`", value="認証済みユーザーを強制参加させます。", inline=False)
    await ctx.send(embed=embed)
...
async def setup_ticket(ctx):
    if not has_admin_role(ctx.author):
        return
    await ctx.message.delete()
    await ctx.send(embed=discord.Embed(title="🎫 お問い合わせ", description="チケットを作成するには下のボタンを押してください。", color=0x57F287), view=TicketView())

@bot.command(name="setup_vending")
async def setup_vending(ctx):
    ...
    items = load_vending()
    vending_embed = discord.Embed(title="🎪 ソースコード自販機", description="以下のメニューまたはボタンから商品をご購入いただけます。", color=0x2b2d31)
    if not items:
...
@bot.command(name="vending_delete")
async def vending_delete(ctx, item_id: str):
    """指定したIDの自販機商品を削除します (例: !vending_delete 1)"""
    if not has_admin_role(ctx.author):
        return
    items = load_vending()
    if item_id in items:
        ...
        save_vending(items)
        perform_backup()
        await ctx.send(f"✅ 商品「{deleted_name}」(ID: {item_id}) を削除しました！", ephemeral=True)
    else:
        await ctx.send(f"❌ 指定されたID (ID: {item_id}) の商品は見つかりませんでした。`!vending_list` でIDを確認してください。", ephemeral=True)
