...
class ItemAddModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send("❌ 価格は半角数字で入力してください。", ephemeral=True)
            return
    ...
    async def stock_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not items:
            ...
            for i_id, info in items.items():
                ...
                field_value = f"価格: {info['price']}円 | 在庫: {stock_str}個 | 販売数: {info.get('sold', 0)}個"
