...
# Render環境やカスタムドメイン、ローカル環境に応じたREDIRECT_URIの設定
...
@app.route("/")
def home():
    oauth_url = f"https://discord.com/api/oauth2/authorize?client_id={CLIENT_ID}&redirect_uri={requests.utils.quote(REDIRECT_URI)}&response_type=code&scope=identify%20guilds.join"
    return f'''
    <html>
...
class VerifyView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)
        url = RENDER_EXTERNAL_URL if RENDER_EXTERNAL_URL else "http://localhost:8080/"
        self.add_item(discord.ui.Button(label="✅ Web認証・ロール受け取り", style=discord.ButtonStyle.link, url=url))
...
async def vending_list(ctx):
    ...
    for i_id, info in items.items():
        embed.add_field(name=str(i_id), value=f"ID: {i_id} | {info['name']}", inline=False)
    await ctx.send(embed=embed)
