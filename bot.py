...
async def setup_announcement(interaction: discord.Interaction, hours: int, message: str):
    ...
    if not scheduled_announcement.is_running():
        scheduled_announcement.start()
    await interaction.response.send_message(f"✅ このチャンネルに {hours}時間おきの定期お知らせを設定しました！", ephemeral=True)
...
@bot.tree.command(name="backup", description="メンバーバックアップを強制実行します。")
@app_commands.checks.has_permissions(administrator=True)
async def backup_command(interaction: discord.Interaction):
    ...
    if success:
        await interaction.followup.send(f"✅ メンバーバックアップを強制実行しました！（登録ユーザー数: {count}人）", ephemeral=True)
    else:
...
async def help_cmd(interaction: discord.Interaction):
    ...
    embed.add_field(name="🎪 自販機機能", value="`/setup_vending` - 自販機パネルを設置します。\n`/vending_list` - 登録商品一覧の確認\n`/vending_delete` - 商品の削除", inline=False)
    embed.add_field(name="📢 定期お知らせ", value="`/setup_announcement` - 指定チャンネルに定期メッセージを設定します。", inline=False)
    embed.add_field(name="👑 実績管理", value="実績チャンネルの投稿数を自動カウントしてチャンネル名を更新します。", inline=False)
    embed.add_field(name="💾 バックアップ", value="`/backup` - メンバーデータのバックアップを手動で強制実行します。", inline=False)
